...
const StationSeed kStations[] = {
    // 电价已下调为"分时电价"的平段价(元/度); 峰=平×1.35, 谷=平×0.55 自动推导
    {"东软软件园充电站(浑南)", "DR", "沈阳市浑南区新秀街2号东软软件园", 123.4512, 41.7098, 0.65},
    {"奥体中心充电站(浑南)",   "AT", "沈阳市浑南区浑南中路与营盘街交汇处", 123.4698, 41.7430, 0.72},
    {"沈阳站前充电站(和平)",   "SZ", "沈阳市和平区胜利南街55号",           123.3969, 41.8010, 0.78},
    {"桃仙机场充电站(浑南)",   "TX", "沈阳市浑南区桃仙国际机场T3停车楼",   123.4779, 41.6410, 0.92},
    {"星海广场充电站(大连)",   "XH", "大连市沙河口区星海广场地下停车场",   121.5953, 38.8800, 0.85},
};
...
bool DBManager::seedData()
{
    ...
    // ---------- 3. 充电站(带缩写, 供电桩编号) ----------
    //    分时电价: price=平段价, 峰段/谷段价按 平×1.35 / 平×0.55 推导(见 DBManager_price.cpp)
    QVector<qint64> stationIds;
    for (const StationSeed &s : kStations) {
        const double peak   = round2(s.price * 1.35);
        const double valley = round2(s.price * 0.55);
        q.prepare(QStringLiteral("INSERT INTO station (name, code_prefix, address, longitude, latitude,"
                                 " price, price_peak, price_valley) "
                                 "VALUES (:n, :cp, :a, :lng, :lat, :p, :pk, :va);"));
        ...
        for (int si = 0; si < kStationCount; ++si) {
            ...
            for (int k = 0; k < count; ++k) {
                ...
                if (canceled) {
                    ...
                    // 费用 = 电量 × 时段加权均价(分时电价: 峰/平/谷按充电分钟切段)
                    const QDateTime startDt = QDateTime::fromString(
                        startTs, QStringLiteral("yyyy-MM-dd HH:mm:ss"));
                    const double avgPrice = avgPriceOf(
                        round2(price * 1.35), price, round2(price * 0.55),
                        startDt, static_cast<qint64>(durMin) * 60);
                    amount = round2(energy * avgPrice);
                    endTs = startDt.addSecs(durMin * 60)
                                .toString(QStringLiteral("yyyy-MM-dd HH:mm:ss"));
                }
